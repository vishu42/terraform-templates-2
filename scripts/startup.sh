...
# Startup script Ubuntu VM instances
...
check_distribution() {
    ...
    if ! command -v lsb_release &> /dev/null; then
        error_exit "lsb_release command not found. This script requires Ubuntu/Debian-based systems."
    fi
    
    # Get distribution information
    local distro_id=$(lsb_release -si 2>/dev/null | tr '[:upper:]' '[:lower:]')
    local distro_codename=$(lsb_release -cs 2>/dev/null)
    
    if [[ -z "$distro_id" ]] || [[ -z "$distro_codename" ]]; then
        error_exit "Could not determine distribution information. This script requires Ubuntu/Debian-based systems."
    fi
    
    # Check if it's Ubuntu or Debian
    if [[ "$distro_id" != "ubuntu" ]] && [[ "$distro_id" != "debian" ]]; then
        error_exit "This script is designed for Ubuntu/Debian-based systems. Detected: $distro_id"
    fi
    
    log_info "Distribution check passed: $distro_id ($distro_codename)"
}
